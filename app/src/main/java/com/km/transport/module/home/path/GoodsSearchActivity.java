...
public class GoodsSearchActivity extends BaseActivity<GoodsSearchPresenter> implements GoodsSearchContract.View {
    ...
    private void loadData(){
        HomeFastPathDto fastPathDto = getIntent().getParcelableExtra("homeFastPath");
        if (fastPathDto != null){
            curCheckedStartCites[0] = new City(-1,-1,fastPathDto.getSourceProvince(),0);
            curCheckedStartCites[1] = new City(-1,-1,fastPathDto.getSourceCity(),0);
            curCheckedStartCites[2] = new City(-1,-1,fastPathDto.getSourceZoning(),0);

            curCheckedEndCites[0] = new City(-1,-1,fastPathDto.getBournProvince(),0);
            curCheckedEndCites[1] = new City(-1,-1,fastPathDto.getBournCity(),0);
            curCheckedEndCites[2] = new City(-1,-1,fastPathDto.getBournZoning(),0);

            tvSiteStart.setText(fastPathDto.getSourceZoning());
            tvSiteEnd.setText(fastPathDto.getBournZoning());
            ...
            float width = TextUtils.isEmpty(fastPathDto.getCarWidth()) ? 0 : Float.parseFloat(fastPathDto.getCarWidth());

            carWidthAdapter.checkEntityByWidth(width);
            carTypeAdapter.checkEntityByType(fastPathDto.getCarType());

            if (width > 0){
                carWidth = fastPathDto.getCarWidth() + "";
            } else {
                carWidth = "";
            }

            if (TextUtils.isEmpty(fastPathDto.getCarType())){
                carType = "";
            } else {
                carType = fastPathDto.getCarType();
            }
            loadHomeGoodsOrders();
        }
    }
    ...
    private void initRvSiteStart() {
        ...
        cityParentStartAdapter = new SelectCityAdapter(this);
    ...
    private void initRvSiteEnd() {
        ...
        cityParentEndAdapter = new SelectCityAdapter(this);
    ...
    private void initTrcukType() {
        ...
        SelectTruckTypeAdapter truckLengthAdapter = new SelectTruckTypeAdapter(this, SelectTruckTypeAdapter.TruckType.LEGNTH);
        ...
        SelectTruckTypeAdapter trcukTypeAdapter = new SelectTruckTypeAdapter(this, SelectTruckTypeAdapter.TruckType.TYPE);
    ...
    private void initGoodsOrderList(){
        ...
        final HomeGoodsOrderAdapter adapter = new HomeGoodsOrderAdapter(this);
